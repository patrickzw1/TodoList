...
function Get-Release {
    $output = (& gh api "repos/$Repository/releases/tags/$Tag" 2>&1 | Out-String).Trim()
    if ($LASTEXITCODE -eq 0) { return $output | ConvertFrom-Json }
    if ($output -match 'HTTP 404|Not Found') { return $null }
    throw "Could not inspect the GitHub Release: $output"
}
