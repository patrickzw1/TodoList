...
Update checks paginate the public GitHub Releases list and compare stable semantic versions numerically across all pages. Drafts, prereleases and invalid or qualified version tags are excluded. The client selects the highest formal version regardless of GitHub's Latest marker, publication time or list order, then reads only that tag's fixed release-download URL for latest.json. It validates the manifest version and requires the package URL to match an uploaded asset from that same release before retaining the existing Tauri signature-verification and installation flow.

Checks have a 30-second total request budget and a 100-page limit. GitHub rate limits, incomplete pagination, unreadable manifests and missing resources for the highest version are reported as check failures; the app neither falls back to an older release nor claims it is current. An installed version equal to or higher than the highest stable release needs no update.
